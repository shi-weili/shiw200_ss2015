...
void main() {
    ...
    float cellOutter = 1.0 - smoothstep(radius, radius, d);
    float cellInner = 1.0 - smoothstep(radius - 0.07, radius, d);
    float cellShape = cellOutter - cellInner * 0.1 * (1.0 - sin(time * 0.5));

    if(d < radius) {
        color = 1.0 - vec3(0.0, cellShape, cellShape);
    } else {
        float background = 1.0 - sin(time * 0.5);
        color = 1.0 - vec3(0.0, background, background);
    }
