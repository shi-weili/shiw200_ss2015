...
void main() {
    ...
    float columnDrift = 30.0 * u_time * (random(vec2(sti.x, 0.0)) * 0.5 + 0.5);
    float brightness = 1.0;
    vec3 cellColor = vec3(1.0);

    if(random(vec2(sti.x, floor(u_time * 1.0))) < 0.05) {

        columnDrift *= -fract(u_time) * 0.0;
        brightness *= 1.5;
        cellColor = vec3(1.0, 0.8, 0.6);
    }

    shift(0.0, columnDrift);

    float radius = snoise01(sti + u_time * 0.5) * 0.5;
    brightness *= pow(radius * random(sti) * 5.0, 2.0);

    color = tint(gCircle(stf, 0.0, radius) * brightness, cellColor);
