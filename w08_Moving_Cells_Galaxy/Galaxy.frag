...
void main() {
    ...
    float rowDrift = u_time * 10.0 * pow(random(vec2(0.0, sti.y)), 2.0);

    if(random(vec2(sti.y, 0.0)) > 0.5) {
    ...
    float radius = snoise01(sti + u_time * 0.5) * 0.5;
    float brightness = pow(radius * 2.0 * random(sti), 3.0) * 20.0;
    ...
    // float distanceFactor = pow(max((bandWidth - abs(mouse.y - sti.y)) / bandWidth, 0.0), 3.0) * 3.0 + 0.02;
    // brightness *= distanceFactor;

    float bandWidth = 100.0 * (abs(mouse.y) / 50.0);
    float distanceFactor = pow(min(abs(sti.y) - bandWidth, 0.0) / bandWidth, 3.0) * 3.0 + 0.02;
    brightness *= distanceFactor;

    float cellShape = gCircle(stf, 0.0, radius) * brightness;
    vec3 cellColor = vec3(0.20, 0.40, 0.60);

    color = tint(cellShape, cellColor);
