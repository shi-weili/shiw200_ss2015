...
void main() {
    ...
    float rowDrift = 30.0 * u_time * (pow(random(vec2(0.0, sti.y)), 2.0) * 0.5 + 0.5);
    ...
    float brightness = pow(radius * 2.0 * snoise01(sti / 100.0), 4.0) * 20.0;

    mouse -= 0.5;
    mouse *= 50.0;
    mouse += 0.5;

    float cellShape = quadrant(stf, 0.5, int(floor(random(sti) * 4.0))) * brightness;
    vec3 cellColor = vec3(0.20, 0.80, 0.60);
