...
void main() {
    ...
    // rotate(snoise(st * 2.0 + time * 1.0) * 12.0);
    rotate(snoise(vec2(st.y * 2.0, 0.0) + time * 0.5) * 100.0);


    shift(vec2(0.3, 0.0));

    // float shape = cross(st, crossWidth);
    float shape = circle(st, 0.1);

    color = vec3(0.1, shape, shape);
