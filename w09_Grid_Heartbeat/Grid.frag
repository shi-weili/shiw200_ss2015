...
void main() {
    ...
    if(t <= 0.1) {
        ...
        cellRadius = t * cellScaleFactor;

    } else if(t <= 0.15) {

        cellRadius = (0.2 - t) * cellScaleFactor;

    } else if(t <= 0.25) {

        cellRadius = (t - 0.1) * cellScaleFactor;

    } else if(t <= 0.45) {

        cellRadius = (0.4 - t) * cellScaleFactor;

    } else if(t <= 0.5) {

        cellRadius = (t - 0.5) * cellScaleFactor;

    }

    cellRadius += 0.20;

    cellRadius *= snoise01(sti * 0.1 + time * 0.6) * 0.7 + 0.5;

    // float cellShape = circle(stf, cellRadius);

    float d = distance(stf, vec2(0.5));
    float angle = atan((stf - vec2(0.5)).y, (stf - vec2(0.5)).x);

    float radiusVariation = snoise(sti * 100.0 + vec2(sin(angle), cos(angle)) * 0.3 + time * 0.5) * 0.15;
    radiusVariation *= clamp(sin(t * PI * 2.0), 0.0, 0.9) + 0.1;

    float radius = cellRadius * (1.0 + radiusVariation);

    float cellOutter = 1.0 - smoothstep(radius - AA, radius + AA, d);
    float cellInner = 1.0 - smoothstep(radius - 0.07, radius + AA, d);
    float cellShape = cellOutter - cellInner * 0.2;


    color = 1.0 - vec3(0.0, cellShape, cellShape);
    // color = vec3(cellShape, 0.0, 0.0);
