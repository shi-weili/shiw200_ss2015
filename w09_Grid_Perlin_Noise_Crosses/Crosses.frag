...
void main() {
    ...
    scale(5.0);
    float crossWidth = 0.57;

    // Red:
    rotateCell(sin(time * 1.0 + snoise(sti)) * 2.0 * PI);
    color.x += cross(stf, crossWidth);

    // Green:
    float cellRotationShift = pow((snoise01(sti + time * 0.2) * 2.0 - 1.0), 3.0) * 0.2 * PI;
    rotateCell(cellRotationShift);
    color.y += cross(stf, crossWidth);

    // Blue:
    rotateCell(cellRotationShift);
    color.z += cross(stf, crossWidth);
