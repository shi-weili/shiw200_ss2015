...
void main() {
    ...
    scale(smoothstep(0.0, 1.0, mouse.y) * 6.0 + 1.0);

    float frameCount = u_tex0Resolution.x / u_tex0Resolution.y;

    vec2 frameResolution = u_tex0Resolution / vec2(frameCount, 1.0);

    stf.x /= frameCount;

    float FPS = (smoothstep(0.0, 1.0, mouse.x) * 100.0 + 10.0);

    float currentFrame = mod(floor(time * FPS), frameCount);

    stf.x += currentFrame / frameCount;

    gl_FragColor = texture2D(u_tex0, stf);
